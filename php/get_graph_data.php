...
// ✅ Last 7 Days Labels and Defaults
...
// ✅ Expense Breakdown by Method
function getMonthlyExpenseMethod($pdo, $user_id) {
    $stmt = $pdo->prepare("
        SELECT payment_method AS label, SUM(amount) AS total 
        FROM transactions 
...
function getYearlyData($pdo, $user_id, $type) {
    ...
    $labels = [];
    $stmt = $pdo->prepare("
        SELECT YEAR(date) AS y, SUM(amount) AS total 
        FROM transactions 
        WHERE user_id = :uid AND type = :type 
